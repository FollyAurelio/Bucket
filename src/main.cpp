...
Mouse mouse;
Keyboard keyboard;
Box box = Box(glm::vec2(140, 3), glm::vec2(200, 100),
			  glm::vec4(0.1f, 0.7f, 0.0f, 1.0f), false);
int main(int argc, char *argv[]) {
	...
	GLFWwindow *window =
		glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Bucket", NULL, NULL);
	if (window == NULL) {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// renderer.drawRectangle(glm::vec2(60.0f, 40.0f),
		// glm::vec2(30.0f, 30.0f), glm::vec3(0.7f, 0.2f, 0.5f), true);
		// renderer.drawRectangle(renderer.rect_shader, glm::vec2(0.0f, 150.0f),
		// glm::vec2(800.0f, 30.0f), glm::vec3(0.7f, 0.2f, 0.5f), true);
...
//	if(glfwGetKey(handle, GLFW_KEY_UP) == GLFW_PRESS)
		renderer.cameraPosition.y += glm::translate(renderer.camera,
glm::vec3(0.0f, 3.0f, 0.0f));
	//if(glfwGetKey(handle, GLFW_KEY_DOWN) == GLFW_REPEAT)
		//renderer.camera = glm::translate(renderer.camera, glm::vec3(0.0f,
-3.0f, 0.0f)); if (glfwGetKey(handle, GLFW_KEY_P) == GLFW_PRESS)
renderer.cameraZoom += 0.1f; if (glfwGetKey(handle, GLFW_KEY_L) == GLFW_PRESS)
...
static void cursorCallback(GLFWwindow *window, double xpos, double ypos) {
	...
	glm::vec4 offsetMouse =
		renderer.inverseCamera * glm::vec4(mouse.position, 0.0f, 1.0f);
